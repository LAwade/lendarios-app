...
//*******************************************************************************************
         ...
         * @author     Stefan Zehnpfennig
         */
        private $runtime = array('socket' => '', 'selected' => false, 'host' => '', 'queryport' => '10011', 'timeout' => 2, 'debug' => array(), 'fileSocket' => '', 'bot_clid' => 'BOT', 'bot_name' => 'BOT Lendário');
        private $data = array();
...
//******************************************************************************************
         ...
         */
        function clientDbList($start = 0, $duration = 800, $count = false) {
            if (!$this->runtime['selected']) {
                return $this->checkSelected();
            }
         ...
         * musicBotCreate
         * 
         * Create a music bot to server.
         * 
         * @param type $cid
         * @return array
         * 
         */
        function musicBotCreate() {
            if (!$this->runtime['selected']) {
                return $this->checkSelected();
            }
            return $this->getData('array', 'musicbotcreate');
        }

        /**
         * musicBotDelete
         * 
         * Delete a music bot to server.
         * 
         * @param int $botid
         * @return array
         * 
         */
        function musicBotDelete($botid) {
            if (!$botid) {
                return $this->generateOutput(false, array('Error: select music id'), false);
            }

            if (!$this->runtime['selected']) {
                return $this->checkSelected();
            }
            return $this->getData('array', 'musicbotdelete bot_id=' . $botid);
        }
         ...
         * 
         */
        function musicBotPlayerInfo($botid) {
            if (!$this->runtime['selected']) {
                return $this->checkSelected();
            }
            return $this->getData('array', "musicbotplayerinfo bot_id=$botid");
        }
         ...
         * botid => bot_id
         * 
         * Actions {action = 0 => stop music bot} 
         *         {action = 1 => play music bot}
         *         {action = 2 => pause music bot} 
         *         {action = 3 => next music bot}
         *         {action = 4 => previous music bot}
         * 
         * @param int $botid
         * @param int $action
         * @return array
         * 
         */
        function musicBotAction($botid, $action) {
            if (!$botid) {
                return $this->generateOutput(false, array('Error: select music id'), false);
            }

            if (!$action) {
                return $this->generateOutput(false, array('Error: select action'), false);
            }

            if (!$this->runtime['selected']) {
                return $this->checkSelected();
            }
            return $this->getData('array', "musicbotplayeraction bot_id={$botid} action={$action}");
        }

        /**
         * playlistList
         * 
         * Apresenta as informações da playlist
         * Ex: playlist_id, playlist_bot_id, playlist_title, playlist_type, playlist_owner_dbid, playlist_owner_name,
         *     needed_power_permission_modify, needed_power_delete, needed_power_song_add, needed_power_song_move,
         *     needed_power_song_remove
         * 
         * @return array
         * 
         */
        function playlistList() {
            ...
            return $this->getData('array', "playlistlist");
        }

        /**
         * playlistInfo
         * Apresenta informações relacionadas a playlist e musicas
         * 
         * Ex: 'playlist_id','playlist_title','playlist_description','playlist_type', 'playlist_owner_dbid'
         *  'playlist_owner_name', 'playlist_max_songs', 'playlist_flag_delete_played', 'playlist_flag_finished'
         *  'playlist_replay_mode','playlist_current_song_id'
         *   
         * @param int $playlistid
         * @return array
         * 
         */
        function playlistInfo($playlistid) {
            if (!$this->runtime['selected']) {
                return $this->checkSelected();
            }
            return $this->getData('array', "playlistinfo playlist_id=$playlistid");
        }
         ...
         *  'playlist_id'
         *  'song_invoker'
         *  'song_previous_song_id' 
         *  'song_url' 
         *  'song_url_loader' 
         *  'song_loaded' 
         *  'song_metadata' => {description":"Official video for \\"Panini\\"} 
         * 
         * @param int $playlistid
         * @return array
         * 
         */
        function playlistSongList($playlistid) {
            if (!$this->runtime['selected']) {
                return $this->checkSelected();
            }
            return $this->getData('multi', "playlistsonglist playlist_id=$playlistid");
        }
         ...
         * 
         */
        function playlistSongAdd($playlistid, $link) {
            if (!$link) {
                return $this->generateOutput(false, array('Error: Informe o link para adicionar em sua playlist!'), false);
            }
            
            if (!$playlistid) {
                return $this->generateOutput(false, array('Error: Informe o id da lista para ser excluida!'), false);
            }
            
            if (!$this->runtime['selected']) {
                return $this->checkSelected();
            }
            return $this->getData('array', "playlistsongadd playlist_id=$playlistid url=$link");
        }
         ...
         * 
         */
        function playlistSongRemove($playlistid, $songid) {
            if (!$songid) {
                return $this->generateOutput(false, array('Error: Informe o id da música para ser excluida!'), false);
            }
            ...
            return $this->getData('array', "playlistsongremove playlist_id=$playlistid song_id=$songid");
        }

        /**
         * playlistEdit
         * 
         * @types = playlist_title, playlist_description, playlist_type, playlist_owner_dbid,
         *          playlist_owner_name, playlist_flag_delete_played, playlist_flag_finished,
         *          playlist_replay_mode, playlist_current_song_id
         * 
         * @param int $playlistid
         * @param int $mode
         ...
         * 
         */
        function playlistEdit($playlistid, $type, $param) {
            
            if (empty($type)) {
                return $this->generateOutput(false, array('Error: O tipo do parametro para ser alterado!'), false);
            }
            
            if (!$this->runtime['selected']) {
                return $this->checkSelected();
            }
            return $this->getData('array', "playlistedit playlist_id=$playlistid $type=$param");
        }
        ...
        private function loadQueryData() {
            ...
            $this->runtime['bot_name'] = 'BOT Lendário'; //$whoAmI['client_nickname'];
         ...
         *  port and initial administrator privilege key. If virtualserver_port is not specified,
         *  the server will test for the first unused UDP port. The first virtual server will be running on UDP port 9987 
         * by default. Subsequently started virtual servers will be running on increasing UDP port numbers.
         * 
         * <b>Input-Array like this:</b>
         * <pre>
         * $data = array();
         ...
         * </pre>
         *
         * <b>Possible properties:</b> VIRTUALSERVER_NAME, VIRTUALSERVER_WELCOMEMESSAGE, 
         * VIRTUALSERVER_MAXCLIENTS, VIRTUALSERVER_PASSWORD, VIRTUALSERVER_HOSTMESSAGE, 
         * VIRTUALSERVER_HOSTMESSAGE_MODE, VIRTUALSERVER_DEFAULT_SERVER_GROUP, 
         * VIRTUALSERVER_DEFAULT_CHANNEL_GROUP, VIRTUALSERVER_DEFAULT_CHANNEL_ADMIN_GROUP,
         * VIRTUALSERVER_MAX_DOWNLOAD_TOTAL_BANDWIDTH, VIRTUALSERVER_MAX_UPLOAD_TOTAL_BANDWIDTH, 
         * VIRTUALSERVER_HOSTBANNER_URL, VIRTUALSERVER_HOSTBANNER_GFX_URL, VIRTUALSERVER_HOSTBANNER_GFX_INTERVAL, 
         ...
         * VIRTUALSERVER_HOSTBANNER_MODE, VIRTUALSERVER_HOSTBUTTON_TOOLTIP, VIRTUALSERVER_HOSTBUTTON_GFX_URL, 
         * VIRTUALSERVER_HOSTBUTTON_URL, VIRTUALSERVER_DOWNLOAD_QUOTA, VIRTUALSERVER_UPLOAD_QUOTA, 
         * VIRTUALSERVER_MACHINE_ID, VIRTUALSERVER_PORT, VIRTUALSERVER_AUTOSTART, VIRTUALSERVER_STATUS, 
         * VIRTUALSERVER_LOG_CLIENT, VIRTUALSERVER_LOG_QUERY, VIRTUALSERVER_LOG_CHANNEL, VIRTUALSERVER_LOG_PERMISSIONS, 
         * VIRTUALSERVER_LOG_SERVER, VIRTUALSERVER_LOG_FILETRANSFER, VIRTUALSERVER_MIN_CLIENT_VERSION, 
         * VIRTUALSERVER_MIN_ANDROID_VERSION, VIRTUALSERVER_MIN_IOS_VERSION, VIRTUALSERVER_MIN_WINPHONE_VERSION, 
         * VIRTUALSERVER_NEEDED_IDENTITY_SECURITY_LEVEL, VIRTUALSERVER_NAME_PHONETIC, VIRTUALSERVER_ICON_ID, 
         * VIRTUALSERVER_RESERVED_SLOTS, VIRTUALSERVER_WEBLIST_ENABLED, VIRTUALSERVER_CODEC_ENCRYPTION_MODE
         *
         * @author     Stefan Zehnpfennig
        ...
        function serverGroupAdd($name, $type = 1) {
            ...
            $this->data = $this->getData('multi', 'servergroupadd name=' . $this->escapeText($name) . ' type=' . $type);
...
//*******************************************************************************************
        ...
        public function convertSecondsToDay($seconds) {
...
//*******************************************************************************************
        ...
        function connect() {
            ...
            if (!$socket) {
                ...
                $tmpVar = fgets($socket);
                if ((strpos($tmpVar, 'TS3') !== false) || (strpos($tmpVar, 'TeaSpeak') !== false ) || (strpos($tmpVar, 'TeamSpeak') !== false )) {
                    $tmpVar = fgets($socket);
